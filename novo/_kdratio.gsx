...
init()
{
    thread novo\_events::addConnectEvent( ::initStats );
	thread novo\_events::addConnectEvent( ::ShowKDRatio );
...
initStats()
{
    if( !isDefined( self.pers[ "kd_initalized" ] ) )
    {
        self.pers[ "shoots" ] = 1;
	    self.pers[ "hits" ] = 1;

        self.pers[ "kd_initalized" ] = true;
    }
}
