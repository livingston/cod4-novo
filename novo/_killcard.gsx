...
init()
{
	// DESIGN NAME | INDEX | COLOR | ALPHA | SHADER | W | H | ALIGNY | Y
	// -- DEFAULT --
	...
	text[0] = "^2CoD^74^2:^3Novo";

	if( isDefined( serverName ) || serverName != "" )
		text[1] = serverName;

	if( isDefined( GetDvar( "scr_novo_emblem_text" ) ) && GetDvar( "scr_novo_emblem_text" ) != "" )
	{
		configuredEmblemTextList = StrTok( GetDvar( "scr_novo_emblem_text" ), ";" );

		preDefinedEmblemTextCount = text.size;

		for( i = 0; i < configuredEmblemTextList.size; i++ )
			text[ i + preDefinedEmblemTextCount ] = configuredEmblemTextList[i];
	}
	...
	if( !isDefined( game[ "emblemchanges" ] ) )
		game[ "emblemchanges" ] = 0;
...
ShowKillCard( attacker, victim, sMeansOfDeath, sWeapon, sHitLoc)
{
 ...
 getDesign( index )
 {
	...
	design = level.designs[ name ];
 	if( isDefined( design ) && isDefined( design[ index ] ))
 		return design[ index ];
 	else
